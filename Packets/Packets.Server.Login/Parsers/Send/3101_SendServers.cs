...
namespace Packets.Server.Login.Parsers.Send
{
    ...
    {
        [ParserAction(PacketType.SendServers)]
        public byte[] Parsing(SendServersModel model)
        {
            FormationPackage formationPackage = new FormationPackage();

            // Отправляем ид аккаунта
            formationPackage.AddInteger(model.AccountId);

            // Отправляем ид сессии
            formationPackage.AddInteger(model.SessionId);
