...
namespace Core.Network
{
    ...
    public class NetworkServer : IDisposable
    {
        ...
        public NetworkServer(IPAddress address, int port) : this(new IPEndPoint(address, port))
        {
        ...
        /// <param name="address">IP address</param>
        /// <param name="port">Port number</param>
        public NetworkServer(string address, int port) : this(new IPEndPoint(IPAddress.Parse(address), port))
        {
        ...
        private void ProcessAccept(SocketAsyncEventArgs e)
        {
            if (e.SocketError == SocketError.Success)
            {
                ...
                NetworkSession session = CreateSession();
        ...
        protected virtual NetworkSession CreateSession()
        {
        ...
        // Server sessions
        protected readonly ConcurrentDictionary<Guid, NetworkSession> Sessions = new ConcurrentDictionary<Guid, NetworkSession>();
        ...
        public NetworkSession FindSession(Guid id)
        {
            // Try to find the required session
            return Sessions.TryGetValue(id, out NetworkSession result) ? result : null;
        }
        ...
        internal void RegisterSession(NetworkSession session)
        {
            // Register a new session
        ...
        {
            // Unregister session by Id
            Sessions.TryRemove(id, out NetworkSession temp);
        }
        ...
        protected virtual void OnConnected(NetworkSession session) { }

        /// <summary>
        /// Handle session disconnected notification
        /// </summary>
        /// <param name="session">Disconnected session</param>
        protected virtual void OnDisconnected(NetworkSession session) { }
        ...
        internal void OnConnectedInternal(NetworkSession session)
        {
            OnConnected(session);
        }

        internal void OnDisconnectedInternal(NetworkSession session)
        {
