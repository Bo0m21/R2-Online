...
namespace Server.Game.Core.Handlers
{
    ...
    public class AttackHandler : IAttackHandler
    {
        ...
        public void BeginAttack(GameSession client, AttackReqModel model)
        {
            ...
            client.CharacterGame.TargetSessionGameId = model.TargetSessionGameId;
            ...
            /*if (model.TargetSessionGameId.Class == (uint)UniqueIdentifierType.Player)
            {
                ...
                Task.Run(() =>
                {
                    ...
                    while (attacker.IsConnected == true && defender.IsConnected == true && attacker.CharacterGame.AttackedUniqueIdentifier != null)
                    {
                        ...
                        if (attacker.CharacterGame.Position.Distance(defender.CharacterGame.Position) > attacker.CharacterGame.DistanceAttack)
                        {
                            attacker.CharacterGame.AttackedUniqueIdentifier = null;
                            break;
                        }
                        ...
                        _attackFactory.SendAttacked(client, attacker.CharacterGame.UniqueIdentifier, defender.CharacterGame.UniqueIdentifier, typeHit, attacker.CharacterGame.Position, -1);

                        foreach (GameSession visible in attacker.CharacterGame.VisibleCharacterGames)
                        {
                            _attackFactory.SendAttacked(visible, attacker.CharacterGame.UniqueIdentifier, defender.CharacterGame.UniqueIdentifier, typeHit, attacker.CharacterGame.Position, -1);
                        }
            ...
            else if (model.TargetSessionGameId.Class == (uint)UniqueIdentifierType.Npc || model.TargetSessionGameId.Class == (uint)UniqueIdentifierType.Monster)
            {
                ...
                Task.Run(() =>
                {
                    ...
                    while (attacker.IsConnected == true && defender != null && attacker.CharacterGame.AttackedUniqueIdentifier != null)
                    {
                        ...
                        _attackFactory.SendAttacked(client, attacker.CharacterGame.UniqueIdentifier, defender.UniqueIdentifier, typeHit, attacker.CharacterGame.Position, defender.Hp);
                ...
                Task.Run(() =>
                {
                    ...
                    var defender = client;
                    if (attacker.Type == Database.Balance.Enums.UnitType.Npc)
                    {
                        return;
                    }
                    ...
                    while (attacker != null && defender.IsConnected == true && attacker.AttackedUniqueIdentifier != null)
                    {
                        ...
                        // Check distance move
                        if (attacker.Position.Distance(attacker.PositionDefault) > attacker.DistanceMove)
                        {
                        ...
                        // Check attack distance
                        if (attacker.Position.Distance(defender.CharacterGame.Position) > attacker.DistanceAttack)
                        {
                            int movingTime = 1000;
                        ...
                        Thread.Sleep(attacker.AttackRate);
                    }
